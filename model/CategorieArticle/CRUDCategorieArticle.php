...
class CRUDCategorieArticle
{
    ...
    function modifCategorie(CategorieArticle $categorieArticle)
    { {
            if (!empty($categorieArticle->getLibelleCategorie()) && !empty($categorieArticle->getId())) {
                $sql = "UPDATE categorie_article SET libelle_categorie=? WHERE id=?";
                $req = $this->pdo->prepare($sql);

                $req->execute(array($categorieArticle->getLibelleCategorie(), $categorieArticle->getId()));
        ...
        header('Location: categorie.php?id=' . $categorieArticle->getId());
        exit;
    }
